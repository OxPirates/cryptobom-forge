...
_X509_ATTRIBUTES_REGEX = re.compile(
    f"({'|'.join(_X509_ATTRIBUTE_NAMES.keys())})(.*['\"].*['\"])", flags=re.IGNORECASE)
_SIGNING_ALGORITHM_REGEX = re.compile(
    f"sign[A-Z\\d_$]*\\(.*({'|'.join(lib_utils.get_algorithms())}).*\\)", flags=re.IGNORECASE)
...
def parse_x509_certificate_details(cbom, finding):
    ...
    component = Component(
        bom_ref=f'cryptography:{CryptoAssetType.CERTIFICATE}:{unique_identifier}',
        name="certificate",#str(unique_identifier),
...
def _generate_distinguished_name(code_snippet):
    ...
    for attribute_name, attribute_value in re.findall(_X509_ATTRIBUTES_REGEX, code_snippet):
        ...
        attribute_value = attribute_value[start_index +
                                          1:len(attribute_value) - 1]
        append(
            f'{_X509_ATTRIBUTE_NAMES[attribute_name.upper()]}={attribute_value}')
